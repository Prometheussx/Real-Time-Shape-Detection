...
import numpy as np
"""
hue-saturation-value  = HSV RENK YAPISI
"""
...
while 1:
    ...
    #AYRIŞTIRILACAK RENGİN ALT VE ÜST SINIRLARINI ATIYORUZ
    lower_color = np.array([lh,ls,lv]) 
    upper_color = np.array([uh,us,uv])
    
    #MASKELEME İLE TEMİZ GÖRSEL ELDE EDİLİMİ
    mask = cv2.inRange(hsv,lower_color,upper_color)
    kernel = np.ones((5,5),np.uint8)
    mask = cv2.erode(mask,kernel)
    
    #KENAR HESAPLAMA VE İSİMLENDRİRME
    contours,_ = cv2.findContours(mask,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    
    #KENAR SAYISINA GRE SINIFLANDIRMA    X
    for cnt in contours:
